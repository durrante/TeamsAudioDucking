...
public partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(AppSettings settings, Action applySettings, Action muteNow, Action restoreNow, Func<string> statusProvider)
    {
        ...
        ChkBoost.IsChecked = settings.BoostCallVolume;
        TxtBoostPercent.Text = settings.CallVolumePercent.ToString();
    ...
    private void BtnSave_Click(object sender, RoutedEventArgs e)
    {
        ...
        _settings.BoostCallVolume = ChkBoost.IsChecked == true;
        if (int.TryParse(TxtBoostPercent.Text.Trim(), out int pct))
            _settings.CallVolumePercent = Math.Clamp(pct, 1, 100);
        TxtBoostPercent.Text = _settings.CallVolumePercent.ToString();
